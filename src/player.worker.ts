...
import {Wrapper, Pointer, IWrapper, AbstractStructType} from 'wasm-ffi';
import {
...
const PlayerWorker = {
    doStep: async ( map: GameMap, robotToMoveIndex: number) => {
        const robots = map.robots.map((robot) => {
            return new RobotStruct({
        ...
        const energyStations = map.energyStations.map((energyStation) => {
           return new EnergyStationStruct({
               position: new PositionStruct({
